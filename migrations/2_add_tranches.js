...
const cAAVE_ADDRESS = "0xe65cdB6479BaC1e22340E4E755fAE7E509EcD06c";

module.exports = async (deployer, network, accounts) => {
  if (network == "development") {
    const factoryOwner = accounts[0];

    adminInst = await JAdminTools.deployed();
    ADMIN_TOOLS_ADDRESS = adminInst.address;
    feeCollInst = await JFeesCollector.deployed();
    FEE_COLLECTOR_ADDRESS = feeCollInst.address;
    trDeplInst = await JTranchesDeployer.deployed();
    COMPOUND_DEPLOYER = trDeplInst.address;
    compInst = await JCompound.deployed();
    COMPOUND_TRANCHE_ADDRESS = compInst.address;
